...
def evaluate_evidential_gru_by_source_sorter(df, model_output_dir=None, seq_length=100):
    ...
    for source in df["source_sorter"].unique():
        ...
        plt.ylabel("SOC")
        plt.ylim(0, 1)
        plt.title(f"Epistemic Uncertainity Analysis on {source}")
        plt.legend()
        plt.grid(True)

        plot_path = os.path.join(plots_dir, f"Epistemic_GRU_Prediction_{source}.png")
        ...
        plt.title(f"Aleatoric Uncertainty Analysis on {source}")
        plt.legend()
        plt.grid(True)
        Aleatoric_plot_path = os.path.join(plots_dir, f"Aleatoric_Uncertainty_{source}.png")
        plt.savefig(Aleatoric_plot_path, dpi=300)
        plt.close()

        # Plot for Total Uncertainty
        plt.figure(figsize=(10, 6))
        plt.plot(time, y_actual, label="True SOC", color="blue", linewidth=1)
        plt.plot(time, pred, label="Predicted SOC", color="red", linestyle="--", linewidth=1)
        ...
        plt.xlabel("Total Time (s)")
        plt.ylabel("Uncertainty")
        plt.title(f"Total Uncertainty Analysis on {source}")
        plt.legend()
        plt.grid(True)
        total_uncertainty_plot_path = os.path.join(plots_dir, f"Total_Uncertainty_{source}.png")
        plt.savefig(total_uncertainty_plot_path, dpi=300)
        plt.close()
